...
//        }
    ...
    public static boolean isItemSimilar(@Nullable ItemStack item1, @Nonnull ItemWrapper item2) {
        return isItemSimilar(item2,item1);
...
//        return !item1.hasItemMeta() && !item2.hasItemMeta();
    ...
    public static boolean isItemSimilar(@Nonnull ItemWrapper item1, @Nullable ItemStack item2) {
        ...
        if (item1.getItemType()!=item2.getType()) {
            return false;
        }
        ItemMeta meta1 = item1.getItemMeta();
        ItemMeta meta2 = item2.getItemMeta();
...
//        }
    ...
    public static boolean isLoreSame(@Nonnull ItemMeta itemMeta1, @Nonnull ItemMeta itemMeta2) {
        return CraftUtils.matchLoreField(itemMeta1,itemMeta2);
...
//        return !itemMeta1.hasLore() && !itemMeta2.hasLore();
    ...
    public static boolean isContainerSame(@Nonnull ItemMeta itemMeta1, @Nonnull ItemMeta itemMeta2) {
        return Objects.equals(itemMeta1.getPersistentDataContainer(),itemMeta2.getPersistentDataContainer());
...
//        return true;
    ...
    public static int stack(@Nullable ItemStack input, @Nonnull ItemWrapper output, int maxAmount) {
        if (!ItemStackUtil.isItemNull(input) && output.getItemAmount() < output.getMaxStackCnt() && ItemStackUtil.isItemSimilar(input, output)) {
            int amount = Math.min(maxAmount, Math.min(input.getAmount(), output.getMaxStackCnt() - output.getItemAmount()));
            input.setAmount(input.getAmount() - amount);
            output.getItemStack().setAmount(output.getItemAmount() + amount);
            return amount;
        }
        return 0;
    }
    ...
    public static int stack(@Nonnull ItemWrapper input, @Nonnull ItemWrapper output, int maxAmount) {
        if (output.getItemAmount() < output.getMaxStackCnt() && ItemStackUtil.isItemSimilar(input, output)) {
            int amount = Math.min(maxAmount, Math.min(input.getItemAmount(), output.getMaxStackCnt() - output.getItemAmount()));
            input.getItemStack().setAmount(input.getItemAmount() - amount);
            output.getItemStack().setAmount(output.getItemAmount() + amount);
            return amount;
        }
        return 0;
